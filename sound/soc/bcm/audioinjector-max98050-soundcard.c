...
#include "../codecs/max98050.h"
#include "../codecs/maxim_ultrasound.h"
...
static int snd_audioinjector_max98050_soundcard_startup
...
static int snd_audioinjector_max98050_soundcard_hw_params
	(struct snd_pcm_substream *substream, struct snd_pcm_hw_params *params)
{
	...
	pr_info("[STEVE] %s sample ch = %d, width = %d\n", __func__, ch, width);
	
	ret = snd_soc_dai_set_tdm_slot(rtd->codec_dai, 1, 0x10, ch, width);
	if (ret < 0)
		return ret;

	pr_info("[STEVE] %s codec tdm slot config ret : %d\n", __func__, ret);

	ret = snd_soc_dai_set_tdm_slot(rtd->cpu_dai, 0x03, 0x03, ch, width);
	if (ret < 0)
		return ret;

	pr_info("[STEVE] %s cpu tdm slot config ret : %d\n", __func__, ret);
#endif

	pr_info("[STEVE] %s sample rate = %d\n", __func__, sr);
...
#endif
	pr_info("[STEVE] %s out. ret:%d\n", __func__, ret);
	return ret;
}

/* machine stream operations */
static struct snd_soc_ops snd_audioinjector_max98050_soundcard_ops = {
	.startup = snd_audioinjector_max98050_soundcard_startup,
	.hw_params = snd_audioinjector_max98050_soundcard_hw_params,
};

static int audioinjector_max98050_soundcard_dai_init
...
SND_SOC_DAILINK_DEFS(audioinjector_max98050,
	DAILINK_COMP_ARRAY(COMP_CPU("bcm2708-i2s.0")),
	DAILINK_COMP_ARRAY(COMP_CODEC("max98050.1-0032", "max98050-aif1")),
	DAILINK_COMP_ARRAY(COMP_PLATFORM("bcm2835-i2s.0")));

static struct snd_soc_dai_link audioinjector_max98050_soundcard_dai[] = {
	{
		.name = "AudioInjector audio",
		.stream_name = "AudioInjector audio",
		.ops = &snd_audioinjector_max98050_soundcard_ops,
		.init = audioinjector_max98050_soundcard_dai_init,
		.dai_fmt = DAI_FMT_BASE,
		SND_SOC_DAILINK_REG(audioinjector_max98050),
	},
};
...
static const struct snd_soc_dapm_route audioinjector_audio_map[] = {
	...
	/* mic is connected to Mic Jack, with max98050 Mic Bias */
//	{"Microphone", NULL, "Mic Bias"},
};

static struct snd_soc_card snd_soc_audioinjector = {
	.name = "audioinjector-max98050-soundcard",
	.dai_link = audioinjector_max98050_soundcard_dai,
	.num_links = ARRAY_SIZE(audioinjector_max98050_soundcard_dai),
...
static int audioinjector_max98050_soundcard_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		struct snd_soc_dai_link *dai =
			&audioinjector_max98050_soundcard_dai[0];
	...
	pr_info("[STEVE] %s --- ret : %d\n", __func__, ret);
	return ret;
}

static const struct of_device_id audioinjector_max98050_soundcard_of_match[] = {
	{ .compatible = "ai,audioinjector-max98050-soundcard", },
	{},
};
MODULE_DEVICE_TABLE(of, audioinjector_max98050_soundcard_of_match);

static struct platform_driver audioinjector_max98050_soundcard_driver = {
	.driver = {
		.name = "audioinjector-max98050",
		.owner = THIS_MODULE,
		.of_match_table = audioinjector_max98050_soundcard_of_match,
	},
	.probe = audioinjector_max98050_soundcard_probe,
};

module_platform_driver(audioinjector_max98050_soundcard_driver);
